...
# Aim - AUC Score ---> which should be greater than o.84
...
setwd("C:\\Users\\Monica\\Desktop\\Projects\\R Projects\\Project4")
...
install.packages("dplyr")   # Keep net connection on while installing
library(dplyr)
...
## score is very less(that is 0.806) so linear model cannot be fitted into the data very well -----------------------------------------------------------------
...
## obtaining auc on testing_data ---------------------------------------------------------------------------------------------------------------
auc(roc(as.numeric(testing_data$left),as.numeric(predict.score)))
## auc score is fine as it is ~ 0.850 so this is the best fitted model here , we achieved our aim as stated above--------------------------------------------------------------------
...
write.csv(final.predict,"Monica_Desai_P4_part2.csv",row.names = F)
